<script lang="ts">
	import { currentPage } from '$lib/shared.svelte';
	...
	function isCurrent(page: string) {
		return currentPage.value === page ? 'bar-icon-active' : '' ;
	}
</script>
...
<aside class="rounded flex flex-col flex-0 items-center pb-2 w-9">
	...
	<a href="/" onclick={() => setActivePage('messages')} aria-label="Messages">
		<svg
			...
			viewBox="0 0 24 24"
			stroke="currentColor"
			class="bar-icon {isCurrent('messages')}"
		>
			<path
	...
	<a href="/people" onclick={() => setActivePage('people')} aria-label="People">
		<svg
			...
			viewBox="0 0 24 24"
			stroke="currentColor"
			class="bar-icon {isCurrent('people')}"
		>
			<path
				stroke-linecap="round"
				stroke-linejoin="round"
	...
	<a href="/settings" aria-label="Settings" onclick={() => setActivePage('settings')}>
		<svg
			...
			viewBox="0 0 24 24"
			stroke="currentColor"
			class="bar-icon {isCurrent('settings')}"
		>
			<path
				stroke-linecap="round"
				stroke-linejoin="round"
	...
	<div class="border-0 border-b-[1px] border-gray-800 w-full a"></div>
	<a href="/nav" aria-label="Logout">
		<svg
			xmlns="http://www.w3.org/2000/svg"
			fill="none"
			viewBox="0 0 24 24"
